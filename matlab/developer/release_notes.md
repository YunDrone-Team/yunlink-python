1.4.5：主线示例连续编号 ex00–ex12；全量检查为 ex12。Release 附带 Typst 编译的使用指南 PDF。

1.4.4：主线示例重新排成连续的 ex00–ex08。全量检查是 ex08。验收不覆盖 Planner。

1.4.3：现场全量检查不再跑航点。验收不覆盖 Planner 接口。
