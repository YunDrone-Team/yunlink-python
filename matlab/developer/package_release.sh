#!/usr/bin/env bash
set -euo pipefail

root="$(cd "$(dirname "$0")/../.." && pwd)"
if [[ -n "${MATLAB:-}" ]]; then
  matlab_bin="$MATLAB"
elif command -v matlab >/dev/null 2>&1; then
  matlab_bin="$(command -v matlab)"
else
  matlab_bin="/Applications/MATLAB_R2026a.app/bin/matlab"
fi
bundle_dir="$root/dist/yunlink-sunray-matlab-1.4.5-bundle"
zip_file="$root/dist/yunlink-sunray-matlab-1.4.5-bundle.zip"
binding_dir="$root/dist/bindings"
sdk_wheel="$root/dist/yunlink_python-1.3.0-py3-none-any.whl"
python_bin="${PYTHON:-python3}"

cd "$root"
mkdir -p "$root/dist"

if [[ ! -f "$sdk_wheel" ]]; then
  "$python_bin" -m pip install -q build
  "$python_bin" -m build --wheel
fi
if [[ ! -f "$sdk_wheel" ]]; then
  echo "missing $sdk_wheel" >&2
  exit 1
fi

rm -rf "$binding_dir"
mkdir -p "$binding_dir"
gh release download v2.0.2 --repo YunDrone-Team/yunlink --dir "$binding_dir" --pattern 'yunlink-*.whl'

"$matlab_bin" -batch "addpath('$root/matlab'); addpath('$root/matlab/developer'); build_release_bundle('$bundle_dir','$binding_dir')"

rm -f "$zip_file"
(
  cd "$root/dist"
  zip -r "yunlink-sunray-matlab-1.4.5-bundle.zip" "yunlink-sunray-matlab-1.4.5-bundle"
)

echo "Created $zip_file"
