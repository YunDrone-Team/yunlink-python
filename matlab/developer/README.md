...
```text
yunlink-sunray-matlab-1.4.4-bundle.zip
```

发布地址：

https://github.com/YunDrone-Team/yunlink-python/releases/tag/matlab-1.4.4
...
4. 生成 `dist/yunlink-sunray-matlab-1.4.4-bundle/` 和对应 zip。

不要把 `dist/`、wheel 或 zip 提交进 git。

GitHub Actions 工作流 `MATLAB Release` 会在 `main` 上的 MATLAB/SDK 变更后自动打包 `.mltbx` 和发布 zip，并更新 `matlab-1.4.4` Release。本仓库的 GitHub Release 只用于 MATLAB 用户包。

本地创建或覆盖 Release：

```bash
gh release upload matlab-1.4.4 \
  dist/yunlink-sunray-matlab-1.4.4-bundle.zip \
  --clobber
```
