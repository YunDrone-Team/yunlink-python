...
% 上一步：ex00_setup。下一步：ex02_connect_and_inspect。
%
...
% 搜索持续秒数。不是端口号。时间越长，越容易等到慢网卡的回复。
timeoutS = 5;

fprintf('正在搜索 Bridge，持续 %.0f 秒（UDP 9697）...\n', timeoutS);
fprintf('若 Windows 弹出防火墙，请允许专用网络。\n');
% yunlink_discover 只听 UDP 广告，不会去连 TCP 9696，也不会 attach。
...
for index = 1:numel(bridges)
    item = bridges(index);
    fprintf('Bridge [%d]\n', index);
    % address 形如 192.168.1.5:9696，这是后面 yunlink_connect 用的。
    fprintf('  连接地址     %s    ← 写入 YUNLINK_ADDRESS=\n', item.address);
    % Bridge ID 是短串 endpoint_uid，多台 Bridge 时给 ex08 用，不是 ip:port。
