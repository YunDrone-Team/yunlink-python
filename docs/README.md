...
地面站探测列表使用 `endpoint_uid@ip:tcp_port` 作为可连接候选，内部再用
`endpoint_uid::entity_uid` 路由设备。二者不能混用，也不能用 IP 地址代替设备 ID。

## 操作边界速查

| 阶段 | 典型调用 | 是否会操作设备 |
| --- | --- | --- |
| 搜索 | `discover()` / `print_discovered_bridges()` | 否，只发送 discovery 查询 |
